...
// Get API base URL dynamically for Vercel deployment
const getAPIBaseURL = () => {
  if (
  ...
  // For production (Vercel), use same host
...
class APIService {
  ...
  async getConversations(limit = 50, skip = 0) {
    try {
      const response = await fetch(
        `${getAPIBaseURL()}/chat/conversations?limit=${limit}&skip=${skip}`,
        {
          method: "GET",
          headers: this.getHeaders(),
        },
  ...
  async getConversation(conversationId) {
    try {
      const response = await fetch(
        `${getAPIBaseURL()}/chat/conversations/${conversationId}`,
        {
          method: "GET",
          headers: this.getHeaders(),
        },
  ...
  async clearConversation(conversationId) {
    try {
      const response = await fetch(
        `${getAPIBaseURL()}/chat/conversations/${conversationId}/clear`,
        {
          method: "POST",
          headers: this.getHeaders(),
        },
